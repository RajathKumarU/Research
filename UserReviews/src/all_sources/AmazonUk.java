...
public class AmazonUk {
	...
	{
		String url = "http://www.amazon.co.uk/product-reviews/B00FP50JAG/ref=cm_cr_pr_btm_link_2?ie=UTF8&showViewpoints=1&sortBy=helpful&pageNumber=";
		FileOutputStream fos = new FileOutputStream("src/all_sources/seagate_HDD.txt", true);
		for( int j=1 ; j<=103;j++)
		{
		try{
			String url1 =url+j;
			...
			{
				Element ele_review = ele_maindiv.get(i).select("span.review-text").first();
				Element ele_rating = ele_maindiv.get(i).select("span.a-icon-alt").first();
				/*if( ele_review == null)
