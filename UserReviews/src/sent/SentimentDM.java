...
public class SentimentDM {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Scanner rat = new Scanner(new File(product+"_val_inval.txt"));
			...
			writtenLine = writtenLine + "@attribute rating numeric\n";
			...
			while (/*int i = 0; i < opinion.length; i++, count++*/sc.hasNext()) {
				...
				writtenLine = writtenLine +","+ rat.nextLine().split("\\|")[0];
				writtenLine =writtenLine+"\n";
				fos.write(writtenLine.getBytes());
	...
	{	
		try {
			...
			//Scanner rat = new Scanner(new File(product+"_val_inval.txt"));
			Scanner rat = new Scanner( new File( product+"_test.txt"));
			// training the opinion classifier
			// String x[] ={"C:/Users/Aniruddha/Downloads/JARs"};/*Dont change
			// this!*/
			// PolarityBasic pb = new PolarityBasic(x);
			...
			writtenLine = writtenLine + "@attribute valid {0,1}\n";
			writtenLine = writtenLine + "\n@data\n\n";
			writtenLine = writtenLine + "\n";
			fos.write(writtenLine.getBytes());
			...
			if( rat.hasNext() && !sc.hasNext())
			{
				while( rat.hasNext() )
				{
					...
					//Add line for appending valid/invalid
					rat.nextLine();
					Zline = Zline+"0";
					fos.write((Zline+"\n").getBytes());
				}
