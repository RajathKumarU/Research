...
public class SentimentFilter {
	...
	{
		String product = "seagate_HDD";
		try{
		FileOutputStream fos = new FileOutputStream("E:/prog/DB/"+product+"_filtered_2.txt",false);
		//Scanner k= new Scanner(new File("E:/prog/DB/"+product+"_ecommerce_all.txt"));
		Scanner k= new Scanner(new File("E:/prog/DB/"+product+".txt"));
		//String rev[] = file.split("\n");
		TextAPIClient client[] = new TextAPIClient[6];
		 client[0] = new TextAPIClient("ba34697c","2e8ee6ebae9c87dbf8640001a0c40fd4");//-->Ani
		 client[1] = new TextAPIClient("f090477c","1e5a83402308292e4332994fa81f5b5f");//--->rajath
		 client[2] = new TextAPIClient("1ed17b9c","83c5ee5d97e3d2790b3f5376191d7eaf");//-->anil
		 client[3] = new TextAPIClient("f2601798","78ddaea3aa5f749c338654fcbf5cef22");//--->anand
		 client[4] = new TextAPIClient("e853721a","38928d046f124744ae9e71c812ac964e"); //--->brad_cooper
		 client[5] = new TextAPIClient("993a9f88","cb2d1a9ae91207cd9dadc8affa5326b8");//-->rajath2
		 int index = 0;
		boolean flag = false;
		String rev="";
		int i =0;
		/*for(int a = 1 ; a<=3988 ; a++)
			k.nextLine();
		/*k.skip(Pattern.compile("(?:.*\\r?\\n|\\r){1000}"));
		k.skip(Pattern.compile("(?:.*\\r?\\n|\\r){1000}"));
		k.skip(Pattern.compile("(?:.*\\r?\\n|\\r){1000}"));k.skip(Pattern.compile("(?:.*\\r?\\n|\\r){1000}"));
		k.skip(Pattern.compile("(?:.*\\r?\\n|\\r){1000}"));
		k.skip(Pattern.compile("(?:.*\\r?\\n|\\r){466}"));*/
		int count = 0;
		while(k.hasNext())
